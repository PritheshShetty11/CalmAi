...
app.post('/send-emergency-email', async (req, res) => {
  try {
    ...
    const msg = {
      ...
      from: process.env.EMAIL_USER, // Should be verified SendGrid sender
...
app.get('/health', (req, res) => {
  res.json({ 
    status: 'healthy',
    service: 'CalmAI Email Service (SendGrid)',
    version: '1.0.0',
    emailConfigured: !!process.env.SENDGRID_API_KEY && !!process.env.EMAIL_USER
  });
});

// Start server
app.listen(PORT, () => {
  console.log(`CalmAI Email Service (SendGrid) running on port ${PORT}`);

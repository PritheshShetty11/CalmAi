...
# Demo mode - Set to False and add your GROQ_API_KEY to enable live mode
DEMO_MODE = True
groq_client = None
...
if not DEMO_MODE:
    ...
    if GROQ_API_KEY:
        ...
        print("GROQ API Key found - Live mode enabled")
    else:
        print("No GROQ API Key found - Falling back to Demo mode")
        DEMO_MODE = True
else:
...
def analyze_with_groq(text):
    if DEMO_MODE or not groq_client:
        ...
        elif any(word in text_lower for word in ["medical", "help", "emergency", "breathing"]):
